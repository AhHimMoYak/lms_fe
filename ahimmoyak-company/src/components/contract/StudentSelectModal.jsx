...
const StudentSelectModal = ({ isOpen, onClose, contract, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedEmployeeNames = employees
        .filter(emp => selectedStudents.includes(emp.id))
        .map(emp => emp.name);

    const requestBody = {
      courseProvideId: contract.courseProvideId,
      employeeUserName: selectedEmployeeNames,
    };

    try {
      const response = await axios.post(
          `${API_URL}/companies/courseProvides/employees`,
          requestBody,
          {
            params: { userId: 2 },
      ...
      onSubmit(selectedEmployeeNames);
      onClose();
    } catch (error) {
      console.error("수강생 목록 제출 실패:", error.response || error.message);
    }
  };
        ...
        <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          <div className="flex justify-between items-center p-6 border-b">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {contract.title} ({contract.attendeeCount}명)
              </p>
            </div>
          ...
          <div className="p-6">
            ...
            <div className="flex justify-between items-center">
              <p className="text-sm text-gray-600">
                선택된 인원: {selectedStudents.length}/{contract.attendeeCount}명
              </p>
